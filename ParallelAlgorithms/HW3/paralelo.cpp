...
inline double alpha(double x, double y){
...
int main(){
    ...
    for(int it = 1; it<=iter; it++){
        double oldrho = rho;
        rho = 0;
        #pragma omp parallel for reduction(+:rho) collapse(2)
        for(int i = 0; i<Nx; i++){
            for(int j = 0; j<Ny; j++){
                rho += r[i][j]*r[i][j];
            }
        ...
        double resto = 0;
        #pragma omp parallel for reduction(+:resto) collapse(2)
        for(int i = 0; i<Nx; i++){
            for(int j = 0; j<Ny; j++){
                resto += r[i][j]*r[i][j];
            }
